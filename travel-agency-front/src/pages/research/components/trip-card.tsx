...
const TripCard = ({ trip }: { trip: Trip }) => {
  ...
  return (
    <Card className="p-0 rounded-xl border hover:shadow-md transition-all duration-200 overflow-hidden bg-white max-h-[350px] cursor-pointer">
      <Link to={`/research/${_id}`} className="block h-full">
        ...
        <CardContent className="px-4 py-3">
          <p className="text-sm text-gray-500 mb-1">
            {regionsCodes.find((item) => item.code === region)?.name}
            &nbsp;{town ? `- ${town}` : ""}
          </p>
